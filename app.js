...
const state = { cards: [], currentIndex: null, recentStudyIds: [], config };
...
function switchView(name) { document.querySelectorAll('.tab').forEach(tab => tab.classList.toggle('is-active', tab.dataset.view === name)); document.querySelectorAll('.view').forEach(view => view.classList.toggle('is-active', view.id === `${name}-view`)); if (name === 'study' && state.cards.length) { chooseStudyCard(); render(); } }
...
function chooseStudyCard() {
  const historySize = Math.min(3, state.cards.length - 1);
  const recentIds = new Set(historySize ? state.recentStudyIds.slice(-historySize) : []);
  const availableCards = state.cards.filter(card => !recentIds.has(card.id));
  const card = availableCards[Math.floor(Math.random() * availableCards.length)];
  state.currentIndex = state.cards.findIndex(item => item.id === card.id);
